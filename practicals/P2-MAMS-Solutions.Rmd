...
<!--- rmarkdown::render("~/courses/bsu/16m=MamsTrialSimulation/20260518-CEN/practicals/P2-MAMS/P2-MAMS+.Rmd") --->
...
* O'Brien & Fleming predefined shapes will be used to define the efficacy boundaries 'a' and 'c' (**ushape**),
* the futility boundary at stage 1, 'b', equals 0 (**lshape** and **lfix**),
...
* **obj**: the output of the **mams.stats()** function (see above),
...
### Under H1 with lapply

```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}

# under the alternative hypothesis
trials_H1 = lapply(1:25000,function(x){
    set.seed(x)
    trialx = mams.stats(nMat, delta = 0.8, delta0 = 0, sd = 2.7)
    mams.trial(trialx, u=design1$u, l=design1$l)
    })
sapply(trials_H1,function(x)sum(x$eff.all[,1]))    
trials_H1[sapply(trials_H1,function(x)sum(x$eff.all[,1]))==1]
```

### Under H0 with a for loop

```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}
# object to fill
trials_H0 = as.list(rep(NA,25000))
# fill
for(rw in 1:25000){
    set.seed(rw)
    trialx = mams.stats(nMat, delta = 0, delta0 = 0, sd = 2.7)
    trials_H0[[rw]] = mams.trial(trialx, u=design1$u, l=design1$l)
    }
```
...
Using the results of Tasks 1/, check that the power and the FamilyWise Error rate (FWER), as defined on slide 19, are respectively close to 90% and 5%. Use confidence intervals to take  into account the uncertainty of the Monte Carlo simulation. 
...
# point estimate:
mean(sapply(trials_H1, function(x)x$eff.T1))
# interval estimate by means of the binomial exact test
binom.test(sum(sapply(trials_H1, function(x)x$eff.T1)),
           length(trials_H1),p=0.9)
```

*COMMENT*:

* with a 95% confidence interval ranging from `r round(binom.test(sum(sapply(trials_H1, function(x)x$eff.T1)),length(trials_H1))$conf.int[1],4)` to `r round(binom.test(sum(sapply(trials_H1, function(x)x$eff.T1)),length(trials_H1))$conf.int[2],4)`, the simulation confirms that the power is close to (not statistically different from) the target 90% power.
...
# point estimate:
mean(sapply(trials_H0, function(x)x$eff.any))
# interval estimate by means of the binomial exact test
binom.test(sum(sapply(trials_H0, function(x)x$eff.any)),
           length(trials_H0),p=0.05)  
           
```

*COMMENT*:

* with a 95% confidence interval ranging from `r round(binom.test(sum(sapply(trials_H0, function(x)x$eff.any)),length(trials_H0))$conf.int[1],3)` to `r round(binom.test(sum(sapply(trials_H0, function(x)x$eff.any)),length(trials_H0))$conf.int[2],3)`, the simulation confirms that the FWEER is close to (not statistically different from) the target 5% FWER.
...
mean(sapply(trials_H0, function(x)x$stage==1))

## probability of early stopping due to efficacy
mean(sapply(trials_H0, function(x)x$stage==1 & x$eff.any))

## probability of early stopping due to overall futility
mean(sapply(trials_H0, function(x)x$stage==1 & !x$eff.any))
```

*COMMENT*:

* the simulation shows that, under the global null hypothesis, we have a high probability of stopping early (p = `r round(mean(sapply(trials_H0, function(x)x$stage==1)),3)`). When doing so, both arms were stopped early for futility in `r round(mean(sapply(trials_H0, function(x)x$stage==1 & !x$eff.any))/mean(sapply(trials_H0, function(x)x$stage==1))*100,3)`% of the cases. 
...
## probability of early stopping 
mean(sapply(trials_H1, function(x)x$stage==1))

## probability of early stopping due to efficacy
mean(sapply(trials_H1, function(x)x$stage==1 & x$eff.any))

## probability of early stopping due to T1 found effective (and best)
mean(sapply(trials_H1, function(x)x$stage==1 & x$eff.T1))
...
* the simulation shows that, under the alternative hypothesis, we have a high probability of stopping early (p = `r round(mean(sapply(trials_H1, function(x)x$stage==1)),3)`). When doing so, T1 was found effective (and best) in `r round(mean(sapply(trials_H1, function(x)x$stage==1 & x$eff.T1))/mean(sapply(trials_H1, function(x)x$stage==1))*100,3)`% of the cases. 
...
the first solution estimate the ESS by taking rge average of the final sample size of each simulated trial:  

```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}

mean(sapply(trials_H1, function(x){     
     sum(apply((x$n * rbind(1,!is.na(x$eff.all)))[,1:x$stage,drop=FALSE],1,max))
     }))
...
the second solution defines the probability associated with each possible sample size outcome and then estimate the ESS by means of a weighted average:

There are 3 scenarios affecting the trial sample size:

* Scenario 1: **both arms stop at stage 1**. This occurs  
    * if T1 and/or T2 are found effective at stage 1,
    * **OR** if both T1 and T2 are dropped for futility at stage 1,
* Scenario 2: **only one arm stops at stage 1**. This occurs
    * if the trial is not stopped for efficacy
    * **AND** if the one arm is dropped for futility,    
* Scenario 3: **both arms continue to stage 2**. 


Let's define the sample size and probability corresponding to each scenario:

```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}

## probability of early stopping 
prob1 = mean(sapply(trials_H1, function(x)x$stage==1))
n1    = nMat[1,1] * 3

## probability of dropping one arm for futility at stage 1
prob2 = mean(sapply(trials_H1, function(x)sum(x$eff.all[,1])==0 & sum(x$fut.all[,1])==1))
n2    = nMat[1,1] * 5

## probability of moving all arms to stage 2
prob3 = 1-prob1-prob2
n3    = nMat[1,1] * 6

## combine information
res = matrix(c(prob1,prob2,prob3,n1,n2,n3),ncol=2,
             dimnames=list(paste0("Scenario",1:3),c("prob","n")))
res             
```

We can now perform the weighted average to obtain the ESS:
...
* both solutions lead to the same ESS estimate,
* solution 2 is harder to implement with larger numbers of treatment arms and/or larger numbers of stages.
...
# we first define a function simulating the drop out in T1 and T2 based on nMat
dropout = function(nMat, prob = c(.15,.025,0)){  
    nmat = nMat - cbind(0,nMat[,-ncol(nMat)])
    for(gw in 1:length(prob)){
        for(jw in 1:ncol(nMat)){
            nmat[gw,jw] = sum(rbinom(nmat[gw,jw],1,1-prob[gw]))
            }
        }    
    t(apply(nmat,1,cumsum))
    }

##
## re-simulate trials under H1 and H0 with drop out
##

# under the alternative hypothesis
trials_H1_dropout = lapply(1:25000,function(x){
    set.seed(x)    
    nMat_dropout = dropout(nMat,prob = c(.15,.025,0))
    trialx = mams.stats(nMat_dropout, delta = 0.8, delta0 = 0, sd = 2.7)
    mams.trial(trialx, u=design1$u, l=design1$l)
    })
# under the global null hypothesis
trials_H0_dropout = lapply(1:25000,function(x){
    set.seed(x)
    nMat_dropout = dropout(nMat,prob = c(.15,.025,0))
    trialx = mams.stats(nMat_dropout, delta = 0, delta0 = 0, sd = 2.7)
    mams.trial(trialx, u=design1$u, l=design1$l)
    })
...
# point estimate:
mean(sapply(trials_H1_dropout, function(x)x$eff.T1))
# interval estimate by means of the binomial exact test
binom.test(sum(sapply(trials_H1_dropout, function(x)x$eff.T1)),
           length(trials_H1_dropout),p=0.9)
...
# point estimate:
mean(sapply(trials_H0_dropout, function(x)x$eff.any))
# interval estimate by means of the binomial exact test
binom.test(sum(sapply(trials_H0_dropout, function(x)x$eff.any)),
           length(trials_H0_dropout),p=0.05)    

```

*COMMENTS*:

* as expected, there is a (significant) decrease in power (from 0.9 to 0.8695), which seems small considering the decrease in sample size in the control group,
* the FWER is now significantly below 0.05 (i.e., slightly conservative change).

The function **new.bounds()** of the MAMS package allows us to update the boundaries of a MAMS study given the observed number of participants per arm at a given stage (check **?new.bounds** for detail). In the following code, we

* define the matrix of expected *cumulated* sample size per arm and stage given the drop-out rate,
* update the boundaries by means of the function **new.bounds()**,
* check the operating characteristics of the updated boundaries by simulation.


```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}

## 
## n matrix given the expected drop out per arm:
##
nMat_EDO     = nMat
nMat_EDO[1,] = round(nMat[1,]*.85)
nMat_EDO[2,] = round(nMat[2,]*.975)

##
## define new boundaries
##
design2 = new.bounds(K=2, J=2, alpha=0.05, nMat = t(nMat_EDO),
                     ushape="obf", lshape="fixed", ufix=NULL, lfix=0, 
                     N=20, parallel=TRUE, print=TRUE)

##
## check the operating characteristics under H1 and H0
## using the new bounds design2$u and design2$l
##

# under the alternative hypothesis
trials_H1_dropoutnb = lapply(1:25000,function(x){
    set.seed(x)    
    nMat_dropout = dropout(nMat,prob = c(.15,.025,0))
    trialx = mams.stats(nMat_dropout, delta = 0.8, delta0 = 0, sd = 2.7)
    mams.trial(trialx, u=design2$u, l=design2$l)
    })
# under the global null hypothesis
trials_H0_dropoutnb = lapply(1:25000,function(x){
    set.seed(x)
    nMat_dropout = dropout(nMat,prob = c(.15,.025,0))
    trialx = mams.stats(nMat_dropout, delta = 0, delta0 = 0, sd = 2.7)
    mams.trial(trialx, u=design2$u, l=design2$l)
...
# point estimate:
mean(sapply(trials_H1_dropoutnb, function(x)x$eff.T1))
# interval estimate by means of the binomial exact test
binom.test(sum(sapply(trials_H1_dropoutnb, function(x)x$eff.T1)),
           length(trials_H1_dropoutnb),p=0.9)

## FWER

# point estimate:
mean(sapply(trials_H0_dropoutnb, function(x)x$eff.any))
# interval estimate by means of the binomial exact test
binom.test(sum(sapply(trials_H0_dropoutnb, function(x)x$eff.any)),
           length(trials_H0_dropoutnb),p=0.05)    
```

*COMMENTS*:

* we can note that, when using the new bounds, the FWER slightly increased (and is now no longer statistically different from the nominal value of 0.05) leading to a slight improvement in power.   
...
```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}
1- mean(sapply(trials_H1, function(x){
        ifelse(x$stage==1 & x$eff.T1, 
               x$zstat["T1","Stage 2"] > design1$u[2] &
               x$zstat["T1","Stage 2"] == max(x$zstat[,"Stage 2"]), 
               NA)}),
        na.rm=TRUE)
```
...
* the simulation shows that the probability of not finding T1 effective (and best) at stage 2 knowing that the trial was stopped at stage 1 due to T1 found effective (and best) is very small (p = `r 1- mean(sapply(trials_H1, function(x){
        ifelse(x$stage==1 & x$eff.T1, 
               x$zstat["T1","Stage 2"] > design1$u[2] &
               x$zstat["T1","Stage 2"] == max(x$zstat[,"Stage 2"]), 
               NA)}),
...
# in all trials in which T1 was found effective, store 
#  i/ the estimated difference in means,
# ii/ the stage.

res.T1eff = t(sapply(trials_H1,function(x){
    if(!is.na(x$eff.all["T1",x$stage])&x$eff.all["T1",x$stage]){
       c(x$diff["T1",x$stage],x$stage)
    }else{rep(NA,2)}
    }))

# in all trials in which T2 was not found effective, store 
#  i/ the estimated difference in means,
# ii/ the stage.

res.T1fut = t(sapply(trials_H1,function(x){
    if(!is.na(x$eff.all["T1",x$stage])&!x$eff.all["T1",x$stage]){
       c(x$diff["T1",x$stage],x$stage)
    }else{rep(NA,2)}
    }))

# display estimated difference in means per stage
# under both scenarios
par(mfrow=c(1,2))
# T1 effective
    boxplot(res.T1eff[,1]~res.T1eff[,2],
            main  = "T1 found effective",
            names = paste0("Stage ",1:2),
            ylab  = "'T1 versus Control' Differences in means",
            xlab  = "Stages"
            )
    # add real parameter value
    abline(h=0.8,col="blue",lty=2)
    axis(2,at=0.8,col.axis="blue",las=2)
    # add estimated parameter value per stage
    est = tapply(res.T1eff[,1],res.T1eff[,2],mean,na.rm=TRUE)
    points(est,col="red",pch=15, cex=.75)
    text(1:2,est,round(est,2),pos=4,col="red")
# T1 not effective
    boxplot(res.T1fut[,1]~res.T1fut[,2],
            main  = "T1 not found effective",
            names = paste0("Stage ",1:2),
            ylab  = "'T1 versus Control' Differences in means",
            xlab  = "Stages"
            )
    ...
    axis(2,at=0.8,col.axis="blue",las=2)
    # add estimated parameter value per stage
    est = tapply(res.T1fut[,1],res.T1fut[,2],mean,na.rm=TRUE)
    points(est,col="red",pch=15, cex=.75)
    text(1:2,est,round(est,2),pos=4,col="red")
```
...
* **Left plot**: The simulation confirms that 
    * when T1 is found effective at stage 1, the difference in means between T1 and control tends to be (strongly) over-estimated 
    *  when finding T1 effective at stage 2, the expected difference in means between T1 and control is slightly underestimated.
* **Right plot**: The simulation shows that, 
    * when T1 is not found effective at stage 2, the difference in means between T1 and control is strongly under-estimated (average around 0.37 instead of 0.8),
    * at stage 1, the bias depends on the reason of early stopping:
        * if the trial was stopped early for futility in both arms, the expected difference in means between T1 and control is smaller than 0 on average (i.e., the Ctrl > T1). 
        * if the trial was stopped early due to T2 found effective, the range of the difference in means between T1 and control has a wide spread (some estimates are close to 0.8 but values below 0 are also possible in such a scenario).
...
# under the alternative hypothesis
trialsplus_H1 = lapply(1:1000,function(x){
    set.seed(x)
    rec   = mams.recruitment(nMat, numberenrolled="rpois(1, 7)",
                             timetodata = "rnorm(1, 40, 5)") 
    stats = mams.stats(rec$nMat,delta=0.8,delta0=0,sd=2.7) 
    trial = mams.trial(stats,u=design1$u,l=design1$l)
    c(trial,rec)                       
    })   
```
...
Let's estimate the study times per stage of each trial (independently of the trial output) and display them:

```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}

# estimate
res.time = t(sapply(trialsplus_H1,function(x){
    apply(x$time,2,max)
    }))
# display
boxplot(res.time,
        main  = "Study times per stage",
        names = paste0("Stage ",1:2),
        ylab  = "Units of time",
        xlab  = "Stages"
        )
# add estimated parameter value per stage
est = apply(res.time,2,mean)
points(est,col="red",pch=15, cex=.75)
text(1:2,est,round(est,2),pos=4,col="red")
```
...
* this is in line with the Figure produced when introducing the **mams.recruitment()** function.

Let's finally estimate the expected number of participants enrolled in the study. This is similar to Task 4/.

```{r message = FALSE, warning = FALSE, include=TRUE, fig.height = 7, fig.width = 10, fig.align = "center"}

mean(sapply(trialsplus_H1, function(x){     
     sum(apply((x$numberenrolled * rbind(1,!is.na(x$eff.all)))[,1:x$stage,drop=FALSE],1,max))
     }))
...
* the ESS estimated in Task 4/ was equal to `r round(mean(sapply(trials_H1, function(x){     
     sum(apply((x$n * rbind(1,!is.na(x$eff.all)))[,1:x$stage,drop=FALSE],1,max))
     })),2)` and suggested that the MAMS design allows a large decrease in sample size in expectation. The same analysis considering the number of patients enrolled suggests that the decrease in sample size actually strongly depends on recruitment characteristics.
